...
#abbrevfile = r"c:\python25\abbreviationsLower.txt"
...
class SLICEngine:
    ...
    def __init__(self, docPath = r'C:\Documents and Settings\User\My Documents\!!!KevinDocs\Research\911 Calls\Transcripts\OperatorStripped'):#os.getcwd() + "\\testDocs"):#r'c:\python25\LiCE\testDocs'):
        self.docPath = docPath
        self.dirList = os.listdir(r'D:\Dropbox\Python26\SLICE')#os.getcwd())
        self.pluginList = []
        self.imp = ""
        ...
        self.headerRowWritten = False #keeps track of the headerrow being written into CSV File"
        self.OutputFileName = "" #added by Sean
...
#________________________________________________________________________#
        ...
        self.sy_dict = {}
        self.SubSyl = ['cial','tia','cius','cious','giu','ion','iou','sia$','.ely$',]
...
#________________________________________________________________________#
    ...
    def setPlugins(self, filename):
        ...
        a = lambda x, y: self.imp.plugin().process(x, y)
        objectType = lambda o: self.imp.plugin().setTextObjectType(o)
        for aPlugin in self.pluginList:            
            ...
            for key in f.keys():
                ...
                self.pluginAttributeDict.update(f)#combine lexical cues into one dictionary
        self.allAttributeDict.update(self.pluginAttributeDict)#update the all Attribute Dict with plugin values    
        self.setDictionaryList = True #set after the dictionarlist has been made
        #print self.dictionaryList
        self.__writeToCSV()
        
    def __writeToCSV(self, destination = os.getcwd()+ "\\output"):# + "\\output"):
        if self.headerRowWritten == False: #make sure the header row only gets written onece
            CSVDictFile = open(destination + "\\" + self.OutputFileName,'wb') #changed by Sean
            CSVWriter = csv.writer(CSVDictFile)
            CSVWriter.writerow(self.dictionaryList)
            self.headerRowWritten = True
            CSVDictFile.close()
        CSVDictFile = open(destination + "\\" + self.OutputFileName,'ab') #changed by Sean
        CSVDictWriter = csv.DictWriter(CSVDictFile, fieldnames = self.dictionaryList)
    ...
    def countWords(self, filename):
        ...
        for line in thetextfile:
            ...
            newline = re.sub(r'[\s+\.\?!,\"\'\%@#\^\(\)\\]','', line) #create a line with no spaces or punc
            self.characters_minus_spaces += len(newline) #counts the number of characters minus spaces,tabs,newlines
    ...
    def get_sy_count(self,word):
        ...
        if (uword == ''): return 0
        if uword in self.sy_dict: return self.sy_dict[uword] # memoize
        """
        if self.cmudict:
