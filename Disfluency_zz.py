...
# Fill pluginDict with plugin results for new linguistic cue        
        ...
        try: #take count and convert to ratio
            pluginDict['Disfluencies'] = self.formatAnswer((intUm + intUh + intEREHAHTTT + intCountRepeats) / self.numWords  )
            pluginDict['Um'] = self.formatAnswer(intUm  / self.numWords)
            pluginDict['Uh'] = self.formatAnswer(intUh / self.numWords)
            pluginDict['UmUh'] = self.formatAnswer((intUm + intUh ) / self.numWords)
            pluginDict['ErEhAhTtt'] = self.formatAnswer(intEREHAHTTT    / self.numWords)
            pluginDict['RepeatPhrasesCount'] =  intCountRepeats  #raw count, this is not a function of how many words in a sentence.
            pluginDict['RepeatPhrasesRatio'] = self.formatAnswer(intCountRepeats  / self.numWords) 
...
#Return the pluginDict. The Dictionary keys will be the column headers.
    ...
    def formatAnswer(self, value):  #Format answer to a percentage with 3 decimals, added by SEan
        value = value * 100
        return '%.3f' % (value)
